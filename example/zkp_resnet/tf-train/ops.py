...
def bottle_resblock2(x, channels, is_training=True, use_bias=True, downsample=False, scope='bottle_resblock', stridess=1):
    ...
    with tf.variable_scope(scope):
        # for shortcut
        orig_x = x
        print('pygraph resblock input channels:{} x.shape:{}'.format(
            channels, x.shape))
        ...
        print('pygraph resblock input channels:{} x.shape:{} after conv_1x1'.format(
            channels, x.shape))
        ...
        print('pygraph resblock input channels:{} x.shape:{} after conv_3x3'.format(
            channels, x.shape))
