...
def share_and_output(plain_ds, file_name = None, plain_only = False):
    """ Interface!
...
def winequality_data_elt(is_regression = False):
    ...
    # split the attributes into two halves
    attr_num = len(attr_v[0])
    print("attr number:", attr_num)
    attr_ds_p0 = []
    attr_ds_p1 = []
    for row in attr_v:
        attr_p0 = row[0: int(attr_num/2)]
        attr_p1 = row[int(attr_num/2):]
        attr_ds_p0.append(attr_p0)
        attr_ds_p1.append(attr_p1)
        
    total_num = len(label_v)
    train_num = int(total_num * (1-TEST_NUM_PERCENT))
    print("total instaces:", total_num)
    attr_all = train_dir + "/all_attr"
    attr_p0_f = train_dir + "/p0_attr"
    attr_p1_f = train_dir + "/p1_attr"
    label_f = train_dir + "/label"

    share_and_output(label_v[0:train_num], label_f, plain_only=True)
    share_and_output(attr_v[0:train_num], attr_all, plain_only=True) 
    share_and_output(attr_ds_p0[0:train_num], attr_p0_f, plain_only=True)
    share_and_output(attr_ds_p1[0:train_num], attr_p1_f, plain_only=True)

    attr_all = test_dir + "/all_attr" 
    attr_p0_f = test_dir + "/p0_attr"
    attr_p1_f = test_dir + "/p1_attr"
    label_f = test_dir + "/label"
    share_and_output(label_v[train_num:], label_f, plain_only=True)
    share_and_output(attr_v[train_num:], attr_all, plain_only=True) 
    share_and_output(attr_ds_p0[train_num:], attr_p0_f, plain_only=True)
    share_and_output(attr_ds_p1[train_num:], attr_p1_f, plain_only=True)
