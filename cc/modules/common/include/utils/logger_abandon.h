...
#include "cc/modules/common/include/utils/safe_queue.h"
...
class Logger {
  class log_stream : public std::ostringstream {
   ...
   public:
    log_stream(Logger& logger, LogLevel level, const char* file, const char* func, int line)
        : logger_(logger) {
      // FORMAT: "[DATE TIME][PROCESS ID][THREAD ID][LOG LEVEL][FILENAME][FUNCTION][LINE] MESSAGE";
...
#endif
      {
        //[kelvin] temp comment mutex lock
        std::unique_lock<std::recursive_mutex> lck(logger_.to_file_mtx_);
 ...
 public:
  ...
  void set_level(int level) { level_ = static_cast<LogLevel>(level); }
  void set_filename(const std::string& filename) {
    if (filename.empty() || (filename == filename_)) {
      return;
    }
    std::unique_lock<std::recursive_mutex> lck(to_file_mtx_);
 ...
 public:
  ...
  std::thread thread_;
  std::recursive_mutex to_file_mtx_;
  log_queue queue_;
  int pid_;
};
