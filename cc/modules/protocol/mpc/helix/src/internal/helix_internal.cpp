...
void HelixInternal::Rsqrt(const vector<Share>& X, vector<Share>& Y) {
  ...
  */
  ...
  vector<Share> sub_res(size);
  vector<Share> relu_prime_res(size);
  vector<Share> select_res(size);
  ...
  for (int i = nr_iters - 1; i >= 0; i--) {
    ...
    Sub(a, b, sub_res);
    DReLU(sub_res, relu_prime_res);
    Scale(relu_prime_res, float_precision);
    Reveal(relu_prime_res, reveal_res, recv_parties);
