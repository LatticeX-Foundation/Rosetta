...
int Sigmoid::funcLinearMPC(
  ...
  for (size_t i = 0; i < size; i++) {
    out[i] = a * x[i];
    funcTruncateElem2PC(out[i], FLOAT_PRECISION, PARTY_A, PARTY_B);
    out[i] += b;
  }
  return 0;
}
  ...
  if (THREE_PC) {
    ...
    if (PRIMARY) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        funcTruncateElem2PC(out[i], 2 * FLOAT_PRECISION, PARTY_A, PARTY_B);
      }
    }
  }
...
int Sigmoid::funcSigmoidG3MPC(
  ...
  if (FOUR_PC) {
    ...
    for (size_t i = 0; i < size; i++)
      A1[i] *= p1; //==> (x * 0.15012)
    if (PRIMARY)
      funcTruncate2PC(A1, FLOAT_PRECISION, size, PARTY_A, PARTY_B);
    ...
    if (PRIMARY)
      funcTruncate2PC(A2, FLOAT_PRECISION, size, PARTY_A, PARTY_B);
