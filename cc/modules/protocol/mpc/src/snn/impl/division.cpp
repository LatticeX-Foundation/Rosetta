  ...
  if (THREE_PC) {
    ...
    for (size_t looper = 1; looper < FLOAT_PRECISION + 1; ++looper) {
      ...
      if (PRIMARY) {
        ...
        // HEREEEEEEE
        // if (partyNum == PARTY_A)
        // 	sendFourVectors<mpc_t>(E_one, F_one, E_two, F_two, adversary(partyNum), size, size,
        // size, size); else 	receiveFourVectors<mpc_t>(temp_E_one, temp_F_one, temp_E_two,
        // temp_F_two, adversary(partyNum), size, size, size, size);
        ...
        // sendTwoVectors<mpc_t>(E_one, F_one, adversary(partyNum), size, size);
        // receiveTwoVectors<mpc_t>(temp_E_one, temp_F_one, adversary(partyNum), size, size);
        // sendTwoVectors<mpc_t>(E_two, F_two, adversary(partyNum), size, size);
        // receiveTwoVectors<mpc_t>(temp_E_two, temp_F_two, adversary(partyNum), size, size);
...
int Select1Of2::mpc_select_1_of_2(
  ...
  ////funcDotProductMPC(x_minus_y, shared_bool, res_prod, size);
...
int XorBit::mpc_xor_bit(
  ...
  ////funcDotProductMPC(shared_x, shared_y, shared_prod, size);
...
int DivisionV2::funcDivisionMPCV2(
  ...
  if (all_less) {
    ////funcDivisionMPC(shared_numerator_vec, shared_denominator_vec, shared_quotient_vec, vec_size);
  ...
  if (THREE_PC) {
    ...
    ////funcComputeMSB3PC(shared_numerator_vec, shared_numer_sign, vec_size);
    GetMpcOpInner(ComputeMSB)->Run3PC(shared_numerator_vec, shared_numer_sign, vec_size);

    vector<mpc_t> shared_denom_sign(vec_size, 0);
    ////funcComputeMSB3PC(shared_denominator_vec, shared_denom_sign, vec_size);
    ...
    ////mpc_select_1_of_2(shared_sign_neg, shared_sign_pos, shared_numer_sign, shared_x_sign, vec_size);
    ...
    ////mpc_select_1_of_2(shared_sign_neg, shared_sign_pos, shared_denom_sign, shared_y_sign, vec_size);
    ...
    ////mpc_xor_bit(shared_numer_sign, shared_denom_sign, quotient_sign_bit, vec_size);
    ...
    ////funcDotProductMPC(shared_numerator_vec, shared_x_sign, numerator_vec, vec_size);
    GetMpcOpInner(DotProduct)->Run(shared_numerator_vec, shared_x_sign, numerator_vec, vec_size);
    ////funcDotProductMPC(shared_denominator_vec, shared_y_sign, denominator_vec, vec_size);
    ...
    ////mpc_select_1_of_2(shared_sign_neg, shared_sign_pos, quotient_sign_bit, quotient_sign, vec_size);
    ...
    for (int i = LEN - 1 - FLOAT_PRECISION; i >= 0; --i) {
      ...
      ////funcRELUPrime3PC(shared_z, shared_beta, vec_size);
    ...
    ////funcDotProductMPC(curr_y, curr_q, curr_prod, vec_size);
    GetMpcOpInner(DotProduct)->Run(curr_y, curr_q, curr_prod, vec_size);
    subtractVectors(numerator_vec, curr_prod, curr_x, vec_size);

    // PART II:  compute the compute the fractinal part,
    // if(PRIMARY)
    //	funcReconstruct2PC(curr_x, vec_size, "curr_x");
...
int FloorDivision::mpc_floor_division(
  ...
  if (THREE_PC) {
    ...
    if (!all_positive) {
      ...
      ////funcComputeMSB3PC(shared_numerator_vec, shared_numer_sign, common_vec_size);
      ...
      ////funcComputeMSB3PC(shared_denominator_vec, shared_denom_sign, common_vec_size);
      ...
      ////mpc_select_1_of_2(shared_sign_neg, shared_sign_pos, shared_numer_sign, shared_x_sign, common_vec_size);
      ...
      ////mpc_select_1_of_2(shared_sign_neg, shared_sign_pos, shared_denom_sign, shared_y_sign, common_vec_size);
      ...
      ////mpc_xor_bit(shared_numer_sign, shared_denom_sign, quotient_sign_bit, common_vec_size);
      ...
      ////funcDotProductMPC(shared_numerator_vec, shared_x_sign, numerator_vec, common_vec_size);
      GetMpcOpInner(DotProduct)->Run(shared_numerator_vec, shared_x_sign, numerator_vec, common_vec_size);
      ////funcDotProductMPC(shared_denominator_vec, shared_y_sign, denominator_vec, common_vec_size);
      GetMpcOpInner(DotProduct)->Run(shared_denominator_vec, shared_y_sign, denominator_vec, common_vec_size);

      ////mpc_select_1_of_2(shared_sign_neg, shared_sign_pos, quotient_sign_bit, quotient_sign, common_vec_size);
    ...
    for (int i = LEN - 1; i >= 0; --i) {
      ...
      ////funcRELUPrime3PC(shared_z, shared_beta, common_vec_size);
      ...
      ////funcSelectShares3PC(candidate_x_update, shared_beta, candidate_x_update, common_vec_size);
      GetMpcOpInner(SelectShares)->Run3PC(candidate_x_update, shared_beta, candidate_x_update, common_vec_size);
      ////funcSelectShares3PC(candidate_q_update, shared_beta, candidate_q_update, common_vec_size);
    ...
    if (all_positive) {
      ...
      ////funcDotProductMPC(curr_q, quotient_sign, shared_quotient_vec, common_vec_size);
