...
ConstPolynomial::ConstPolynomial(double init_start, double init_end,
...
void PolyConfFactory::func_register(const std::string& func_name, vector<ConstPolynomial>* approx_polys) {
...
bool PolyConfFactory::get_func_polys(const std::string& func_name, vector<ConstPolynomial>** approx_polys) {
...
struct LogFuncRegistrar {
    ...
    LogFuncRegistrar() {
		...
		ConstPolynomial	log_v2_appro_poly_1 = ConstPolynomial(0.0001, 
															0.05, 
												FUNC_APPROX_LOG_OPTION_B_1);
		ConstPolynomial	log_v2_appro_poly_2 = ConstPolynomial(0.05, 
															1.2, 
												FUNC_APPROX_LOG_OPTION_B_2);
		// Note: Attention! The result may become worse when X > 10.
		//		It will be better than ZERO. So we set it to be a VERY BIG NUMBER!
		ConstPolynomial	log_v2_appro_poly_3 = ConstPolynomial(1.2, 
															10000, // 2^40
												FUNC_APPROX_LOG_OPTION_B_3);
		log_v2_vec = new vector<ConstPolynomial>({log_v2_appro_poly_1,
																		log_v2_appro_poly_2,
																		log_v2_appro_poly_3});
    ...
    CELogFuncRegistrar() {
		...
		*/
		const std::vector<std::vector<double>> MPC_CE_LOG_OPTION = \
				...
				{3, -0.01761977},
				{4, 0.00083065}};
		ConstPolynomial	ce_log_appro_poly = ConstPolynomial(0, 7.1, 
												MPC_CE_LOG_OPTION);
