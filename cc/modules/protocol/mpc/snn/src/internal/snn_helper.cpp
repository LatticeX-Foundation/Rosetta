...
small_mpc_t kAdditionModPrime[PRIME_NUMBER][PRIME_NUMBER];
small_mpc_t kMultiplicationModPrime[PRIME_NUMBER][PRIME_NUMBER];
...
void init_mod_prime_tables() {
  ...
  for (int i = 0; i < PRIME_NUMBER; ++i)
    for (int j = 0; j < PRIME_NUMBER; ++j) {
      kAdditionModPrime[i][j] = (i + j) % PRIME_NUMBER;
      kMultiplicationModPrime[i][j] = (i * j) % PRIME_NUMBER;
    }
}
