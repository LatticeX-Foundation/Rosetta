...
int _inner_proof_and_check(const ZkIntFp* a, const ZkIntFp* a_prime, size_t size, uint64_t diff_bound) {
  ...
  log_debug << "begin arith2bool<BoolIO<NetIO>> for ZK MUL with size:" << size;
  arith2bool<BoolIO<NetIO>>(bin_diff.data(), (IntFp*)field_diff.data(), size);
  // sync_zk_bool<BoolIO<NetIO>>();
  auto switch_time = time_from(start);
  log_debug << "arith2bool<BoolIO<NetIO>> for ZK MUL ok, costing " << switch_time/1000 << " for party " << zk_party_id << endl;
...
int _inner_div_proof_and_check(const std::vector<ZkIntFp>& a_zks, const std::vector<ZkIntFp>& b_zks, const std::vector<ZkIntFp>& c_zks) {
  ...
  for(int i = 0; i < size; ++i) {
    ...
    // encoded as PR-b
    neg_b_zks[i] = -b_zks[i];
    log_debug << i << "-th proof data: " << (a_scaled_zks[i].value >> 64) << " VS " <<  (bc_zks[i].value >> 64);
  }
  a_scaled_zks.clear();
  bc_zks.clear();
  ...
  //                  this will be a little costly.
  auto start = clock_start();
  log_debug << "begin arith2bool<BoolIO<NetIO>> for ZK DIV";
  arith2bool<BoolIO<NetIO>>(bin_diff.data(), (IntFp*)field_diff.data(), size);
  arith2bool<BoolIO<NetIO>>(bin_b.data(), (IntFp*)b_zks.data(), size);
  arith2bool<BoolIO<NetIO>>(bin_neg_b.data(), (IntFp*)neg_b_zks.data(), size);
  // sync_zk_bool<BoolIO<NetIO>>();
  auto switch_time = time_from(start);
  log_debug << "arith2bool<BoolIO<NetIO>> for ZK DIV ok, costing " << switch_time/1000 << " for party " << zk_party_id << endl;
...
int _proof_and_check_inv(const std::vector<ZkIntFp>& a, const std::vector<ZkIntFp>& a_inv) {
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  auto start = clock_start();
  log_debug << "begin arith2bool<BoolIO<NetIO>> for ZK INV";
  arith2bool<BoolIO<NetIO>>(bin_diff.data(), (IntFp*)res_zks.data(), size);
  arith2bool<BoolIO<NetIO>>(bin_a.data(), (IntFp*)a.data(), size);
  arith2bool<BoolIO<NetIO>>(bin_neg_a.data(), (IntFp*)neg_a_zks.data(), size);
  // sync_zk_bool<BoolIO<NetIO>>();
  auto switch_time = time_from(start);
  log_debug << "arith2bool<BoolIO<NetIO>> for ZK INV ok, costing " << switch_time/1000 << " for party" << zk_party_id << endl;
