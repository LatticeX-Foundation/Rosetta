...
int WolverineOpsImpl::PrivateInput(
  ...
  vector<string>& out_x) {
  log_debug << "calling WolverineOpsImpl::PrivateInput" << endl;
  ...
  if (zk_input_party_id != ALICE) {
    log_error << " In ZK, only private input from Prover is allowed!";
...
int WolverineOpsImpl::PublicInput(
  ...
  log_debug << "calling WolverineOpsImpl::PublicInput" << endl;
  int size = in_x.size();
  out_x.clear();
  out_x.resize(size);
  vector<uint64_t> field_elements(size);
...
int WolverineOpsImpl::Broadcast(int from_party, const char* msg, char* result, size_t size) {
  ...
  if (from_party + 1 == zk_party_id) {
    ((ZKFpExecPrv<BoolIO<NetIO>>*)(ZKFpExec::zk_exec))->io->send_data((const void*)msg, size);
  } else {
    ((ZKFpExecPrv<BoolIO<NetIO>>*)(ZKFpExec::zk_exec))->io->recv_data((void*)result, size);
  }
  return 0;
}
...
int WolverineOpsImpl::Add(
  ...
  #if DEBUG_MODE
  for(int i = 0; i < size; ++i) {
    log_debug << i <<"-th add : " << fp_a[i] << " + " << fp_b[i] << "-->" << fp_c[i];
  }
  #endif
  ...
  if (a_scale != b_scale && _OUTPUT_WITH_HIGH_SCALE) {
      log_info << "Adjust scale for Op Add:" << a_scale << " VS " << b_scale;
  }
  ...
  #if DEBUG_MODE
  for(int i = 0; i < size; ++i) {
    log_debug << i <<"-th add : " << a_zks[i].value << " + " << b_zks[i].value << "-->" << c_zks[i].value;
  }

  #endif

  convert_mac_to_string(c_zks, output, humanable, output_scale_multiplier);
  log_debug << "Wolverine Add ok. <----";
  return 0;
}
...
int WolverineOpsImpl::Matmul(
  ...
  log_info << "Wolverine Matmul m, k, n: " << m << ", " << k << ", " << n
  ...
  if(a_scale != 1) {
    log_error << "scale value for Matmul left input:" << a_scale;
  ...
  if (rh_is_const) {
    ...
    int b_scale = convert_string_to_mac(b.data(), (uint64_t*)b_fps.data(), b.size());
      // assert the input must be well-scaled
    if(b_scale != 1) {
      log_error << "scale value for Matmul Right input:" << b_scale;
  ...
  if (_OUTPUT_WITH_HIGH_SCALE) {
    log_info << "SCALE DOUBLE in Matmul!";
  ...
  //! cout << "c_fps: \n" << c_fps << endl;
  log_info << "wolverine_matmul_fp_with_proof elapse (us):  " << timer_proof.ns_elapse() / 1000
           << " us, proof accumulate: " << matmul_proof_time / 1000 << " us." << endl;
  log_info << "Wolverine Matmul ok. <---- " << zk_party_id << ",  m, k, n: " << m << ", " << k
           << ", " << n;
  return 0;
#endif
...
int WolverineOpsImpl::Sub(
  ...
  #if DEBUG_MODE
  for(int i = 0; i < size; ++i) {
    log_debug << i <<"-th sub : " << fp_a[i] << " + " << fp_b[i] << "-->" << fp_c[i];
  }
  #endif
  ...
  // todo: refine code
  if (a_scale != b_scale && _OUTPUT_WITH_HIGH_SCALE) {
    log_info << "Adjust scale for Op Sub:" << a_scale << " VS " << b_scale;
  }
  bool need_scale_align = (a_scale == 2 || b_scale ==2);
  for (int i = 0; i < size; ++i) {
    // adjust scale 
...
int WolverineOpsImpl::Mul(
  ...
  if(need_broadcast) {
    log_info << "expand input b in MUL";
    assert(size % b_size == 0);
    for(int i = 0; i< size; ++i) {
      fp_expanded_b[i] = fp_b[i % b_size];
    }
  } else {
    log_info << "no need to expend in MUL";
    assert(size == b_size);
  ...
  if (rh_is_const) {
    ...
    if(a_scale == 1 && b_scale == 1 && _OUTPUT_WITH_HIGH_SCALE) {
      log_info << "SCALE DOUBLE in Mul!";
      for (auto i = 0; i < size; i++) {
        fp_c[i] = mult_mod(fp_a[i], fp_expanded_b[i]);
        log_debug << i << "-th " << fp_a[i] << " * " << fp_expanded_b[i] << "-->" << fp_c[i];
      }
      output_scale_multiplier = 2;
    } else {
      if((a_scale > 1 || b_scale > 1) && _OUTPUT_WITH_HIGH_SCALE) {
        log_warn << "SCALE BACK in Mul :" << a_scale << " VS " << b_scale;
      }
  ...
  for(int i =0; i< size; ++i) {
    log_info << "zk Mul: " << fp_a[i] << " * " << fp_expanded_b[i] << "-->" << fp_c[i] << endl;
  }
  #endif
  ...
  a_scale = convert_string_to_mac(a, a_zks);
  if (rh_is_const) {
    log_info << "Input in Mul set rh_is_const attr!";
    ...
    if(a_scale == 1 && _OUTPUT_WITH_HIGH_SCALE) {
      log_info << "SCALE DOUBLE in Mul with rh_is_const!";
      for (auto i = 0; i < size; i++) {
        if(!need_broadcast) {
          c_zks[i] = a_zks[i] * fb[i];
        } else {
          log_debug << "debug broadcast stub A in rh_is_const!";
          int d = i % b_size;
      ...
      output_scale_multiplier = 2;
        ...
        if(a_scale > 1 && _OUTPUT_WITH_HIGH_SCALE) {
          log_warn << "SCALE BACK in Mul with rh_is_const!" << a_scale;
      }
      ...
      if (_OUTPUT_WITH_HIGH_SCALE) {
        ...
        for (auto i = 0; i < size; i++) {
          if(!need_broadcast) {
            ...
            log_debug << "debug broadcast stub B in both scaled case!";
            int d = i % b_size;
  ...
  #if DEBUG_MODE
  for(int i =0; i< size; ++i) {
    log_info << "zk Mul: " << a_zks[i].value << " * " << b_zks[i%b_size].value << "-->" << c_zks[i].value << endl;
  }
  #endif

  log_debug << "Wolverine Mul ok. <----";
  return ret;
}
...
int WolverineOpsImpl::Mean(
  ...
  if (attr_info && attr_info->count("rows") > 0 && attr_info->count("cols") > 0) {
    ...
    log_error << "please fill rows,cols for mean(x, rows, cols) ";
    return -1;
  }

  log_info << "Wolverine Mean ----> rows: " << rows << ", cols: " << cols;
  int a_scale = 1;
#if LOCAL_SIMULATE
  ...
  // log_info << "inv_fps: " << inv_fp[0].value << ", result: " << result_fps[i].value << ", fl: " << fl << " cast_float: " << (float((int64_t)(c_fps[i]))) << ", c_fps: " << c_fps[i];
 
  convert_mac_to_string(c_fps, output, humanable, a_scale);
  log_info << "Wolverine Mean ok. <---- a_scale: " << a_scale;
  return 0;
#endif//
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(bc.data(), (ZkIntFp*)c_zks.data(), size);
  ...
  for (size_t i = 0; i < bc.size(); i++) {
    ...
    // log_info << "inv_fps: " << INV_N[0] << ", result: " << (uint64_t)(results[i].value >> 64);//<< ", fl: " << fl << ", c_fps: " << c_fps[i];
  }

  // sync_zk_bool<BoolIO<NetIO>>();
  bool2arith<BoolIO<NetIO>>(c_zks.data(), bc.data(), bc.size());
  sync_zk_bool<BoolIO<NetIO>>();
...
int WolverineOpsImpl::Max(
  ...
  if (attr_info && attr_info->count("rows") > 0 && attr_info->count("cols") > 0) {
    ...
    log_error << "please fill rows,cols for wolverine_max(x, rows, cols) ";
    return -1;
  }
  int a_scale = 0;
  log_debug << "Wolverine Max ----> rows: " << rows << ", cols: " << cols;
...
int WolverineOpsImpl::Relu(
  ...
  log_info << "-----> Wolverine Relu, scale count: " << scale_count << " element size: " << a.size();
...
int WolverineOpsImpl::Div(
  ...
  if (rh_is_const) {
    log_debug << "Input in Div set rh_is_const attr!";
    size = b.size();
...
int WolverineOpsImpl::Sqrt(
  ...
  vector<string>& output,
  const attr_type* attr_info) {
  log_debug << "calling WolverineOpsImpl::Sqrt string" << endl;
  int size = a.size();
  output.clear();
  output.resize(size);
  vector<ZkIntFp> a_zks(size);
...
int WolverineOpsImpl::Rsqrt(
  ...
  vector<string>& output,
  const attr_type* attr_info) {
  log_debug << "calling WolverineOpsImpl::Rsqrt string" << endl;
  ...
  int a_scale  = 1;
#if LOCAL_SIMULATE
  log_info << "calling WolverineOpsImpl::Rsqrt Local simulation!" << endl;
  vector<uint64_t> fp_a(size);
  vector<uint64_t> fp_c(size);
  a_scale = convert_string_to_mac(a.data(), fp_a.data(), size);
  ...
  int gate_num = 0;
  sync_zk_bool<BoolIO<NetIO>>();
  log_debug << "DEBUG STUB After sync_zk_bool" << endl;
  vector<Integer> bin_a(size);
  arith2bool<BoolIO<NetIO>>(bin_a.data(), (IntFp*)a_zks.data(), size);
  log_debug << "DEBUG STUB 3: arith2bool" << endl;
  ...
  log_debug << "Add Gate num for RSqrt: " << CircuitExecution::circ_exec->num_and() - gate_num << endl;
  bool2arith<BoolIO<NetIO>>(c_zks.data(), bin_a.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
  log_debug << "DEBUG STUB 3: bool2arith" << endl;
  convert_mac_to_string(c_zks, output);
  #if DEBUG_MODE
  for(int i =0; i< size; ++i) {
    log_info << "zk Qsqrt: " << a_zks[i].value << "-->" << c_zks[i].value << endl;
  }
  #endif
  ...
  //     }
  //     log_debug << i << "-th:" << plain_a[i] << "=>" << plain_c[i] << endl;
  //   }
  ...
  //   log_debug << "Float Rsqrt: " << plain_c[i] << "-->" << field_elements[i] << " --> " << output[i] << endl;
  ...
  //   log_debug << "get "<< i << "-th ZK proof Done" << endl;
  //   #endif
  // }
...
int WolverineOpsImpl::Invert(
  ...
  vector<string>& output,
  const attr_type* attr_info) {
  log_debug << "-----> Wolverine Invert" << endl;
...
#endif
  log_debug << "Wolverine Exp ok" << endl;
  return ret;
}
...
int WolverineOpsImpl::Reveal(
  ...
  vector<string>& output,
  const attr_type* attr_info) {
  log_debug << "calling WolverineOpsImpl::Reveal string" << endl;
  int vec_size = a.size();
  output.clear();
...
int WolverineOpsImpl::Reveal(
  ...
  const attr_type* attr_info) {
  log_debug << "calling WolverineOpsImpl::Reveal[double]" << endl;
  ...
  log_debug << "reveal input scale:" << a_scale << ":" << fp_a[0];
  zk_decode_(fp_a, float_a, a_scale);
  for(int i = 0; i < vec_size; ++i) {
  ...
  log_info << "succeed in verifying zk!! check: " << status;
  delete[] all_x;
  delete[] mat_c;
#else
  vector<uint64_t> output_x(vec_size);
  batch_reveal((IntFp*)(inner_a.data()), output_x.data(), vec_size);
  log_info << "succeed in verifying zk!!";
#endif
  ...
  //   if (zk_party_id == ALICE) {
  //     log_info << "Wow, I'm prover, and am exposing the value to show you now!";
  //     output.clear();
  ...
  //   if(ret) {
  //     log_info << "succeed in verifying zk!!" << endl;
  //     return 0;
  //   } else {
  //     return 1;
  //   }
...
int WolverineOpsImpl::Softmax(
  ...
  if (attr_info->count("rows") > 0 && attr_info->count("cols") > 0) {
    ...
    log_debug << "softmax rows:" << rows << ", cols:" << cols << endl;
  } else {
...
#if LOCAL_SIMULATE
  {
    vector<uint64_t> fp_a(size);
    int a_scale = convert_string_to_mac(a.data(), fp_a.data(), size);
    log_info << "Softmax input scale: " << a_scale;
    vector<float> float_a(size);
    zk_decode_(fp_a, float_a, a_scale);
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(a_int.data(), (IntFp*)a_zks.data(), size);
  ...
  for (int i = 0; i < rows; i++) {
    ...
    // Bool to Arithmetic
    bool2arith<BoolIO<NetIO>>((IntFp*)res_zks.data(), res_int.data(), cols);
    sync_zk_bool<BoolIO<NetIO>>();
  ...
  log_debug << "softmax ok, costing (us) " << switch_time << " for party" << zk_party_id << endl;
  log_debug << "-----> Wolverine Softmax ok";
  return 0;
}
...
int WolverineOpsImpl::Sigmoid(
  ...
  vector<Integer> a_int(size, Integer(ZK_INT_LEN, 0, PUBLIC));
  int scale_num = convert_string_to_mac(a, a_zks);
  log_debug << "(sigmoid input)scale number:" << scale_num;
  ...
  // Bool to Arithmetic
  bool2arith<BoolIO<NetIO>>((IntFp*)a_zks.data(), a_int.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
  convert_mac_to_string(a_zks, output);

  log_info << "-----> Wolverine Sigmoid ok";
  return 0;
}
...
******************************************************************/
  ...
  log_info << "-----> Wolverine Exp to Mul_const ..." << endl;
...
int WolverineOpsImpl::Exp_Exact(const vector<string>& a, vector<string>& output, const attr_type* attr_info) {
  ...
  bool2arith<BoolIO<NetIO>>((IntFp*)res_Int.data(), a_Int.data(), size); 
  sync_zk_bool<BoolIO<NetIO>>();
...
int WolverineOpsImpl::_Inner_Invert(const vector<ZkIntFp>& a, vector<ZkIntFp>& output) {
  ...
  vector<Integer> a_Int(size);
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(a_Int.data(), (IntFp*)a.data(), size);
  ...
  // Step 3: Bool to Arithmetic
  bool2arith<BoolIO<NetIO>>((IntFp*)output.data(), a_Int.data(), size); 
  sync_zk_bool<BoolIO<NetIO>>();
  return 0;
}
...
int WolverineOpsImpl::_Inner_Mul(const vector<uint64_t>& a, const vector<uint64_t>& b, vector<uint64_t>& output, int a_scale, int b_scale) {
  ...
  if(a_scale == 1 && b_scale == 1) {
    log_info << "sim truncation case 1 : 1 VS 1";
    for(int i = 0; i < size; ++i) {
    ...
    log_info << "sim truncation case 2: " << to_string(a_scale) << " VS " << to_string(b_scale);
    for(int i = 0; i < size; ++i) {
      fp_c[i] = mult_mod(a[i], b[i]);
      log_debug << "Fp_res:" << fp_c[i];
    }
    zk_decode_(fp_c, float_c, a_scale + b_scale);
    zk_encode_float32(float_c, output);
  } else {
    log_info << "sim truncation case 3: 2 VS 2";
...
int WolverineOpsImpl::_Inner_Mul(const vector<ZkIntFp>& a, const vector<T>& b, vector<ZkIntFp>& output, int extra_scale_multiplier, bool rhs_broadcast) {
  ...
  if (extra_scale_multiplier == 3) {
    log_warn << "debug both inputs double-scaled";
    ...
    arith2bool<BoolIO<NetIO>>(a_bool.data(), (IntFp*)a.data(), size);
    arith2bool<BoolIO<NetIO>>(b_bool.data(), (IntFp*)b.data(), b_size);
    Float a_float(0., PUBLIC);
    Float b_float(0., PUBLIC);
    float2int_counter += size + b_size;
    int2float_counter += size + b_size;
    log_info << "MUl for double scaled inputs, part 1: scale down both operands with size:" << size << " VS " << b_size;
    for (auto i = 0; i < size; i++) {
      SimpleTimer timer_b2f;
    ...
    bool2arith<BoolIO<NetIO>>((IntFp*)scaled_a.data(), a_prime_res.data(), size);
    bool2arith<BoolIO<NetIO>>((IntFp*)scaled_b.data(), b_prime_res.data(), b_size);
    sync_zk_bool<BoolIO<NetIO>>(); 
    // 2. Mul in Fp:
    for (auto i = 0; i < size; i++) {
      if(!rhs_broadcast) {
          ...
          log_debug << "debug broadcast stub C in double scaled case!";
          int d = i % b_size;
          double_scaled_c[i] = scaled_a[i] * scaled_b[d];
        }
    }
    // 3. Truncate again:
    log_info << "MUl for double scaled inputs, part 2: scale down result with size:" << size;
    sync_zk_bool<BoolIO<NetIO>>();
    arith2bool<BoolIO<NetIO>>(a_bool.data(), (IntFp*)double_scaled_c.data(), size);
    float2int_counter += size;
    int2float_counter += size;
    for (auto i = 0; i < size; i++) {
      SimpleTimer timer_b2f;
    ...
    log_info << "begin b2a:" << size;
    bool2arith<BoolIO<NetIO>>((IntFp*)output.data(), a_prime_res.data(), size);
    sync_zk_bool<BoolIO<NetIO>>();
    return 0;
  }
  ...
  for (auto i = 0; i < size; i++) {
    if(!rhs_broadcast) {
      ...
      log_debug << "debug broadcast stub D !";
      int d = i % b_size;
      c_zks[i] = a[i] * b[d];
    }
  }
  ...
  float2int_counter += size;
  int2float_counter += size;
  log_warn << "extra Truncation with Float in _Mul_:" << extra_scale_multiplier << endl;
  for (auto i = 0; i < size; i++) {
    SimpleTimer timer_b2f;
  ...
  bool2arith<BoolIO<NetIO>>((IntFp*)output.data(), c_res.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
  return 0;
}
...
int WolverineOpsImpl::_Inner_Div(const vector<ZkIntFp>& a, const vector<ZkIntFp>& b, vector<ZkIntFp>& output) {
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(a_int.data(), (IntFp*)a.data(), size);
  arith2bool<BoolIO<NetIO>>(b_int.data(), (IntFp*)b.data(), size);
  ...
  bool2arith<BoolIO<NetIO>>((IntFp*)output.data(), c_int.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
  return 0;
}
...
int WolverineOpsImpl::_Inner_Sqrt(const vector<ZkIntFp>& a, vector<ZkIntFp>& output) {
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  vector<Integer> bin_a(size);
  arith2bool<BoolIO<NetIO>>(bin_a.data(), (IntFp*)a.data(), size);
  ...
  log_debug << "Add Gate num for Sqrt: " << CircuitExecution::circ_exec->num_and() - gate_num << endl;

  bool2arith<BoolIO<NetIO>>(output.data(), bin_a.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
  return 0;
}
