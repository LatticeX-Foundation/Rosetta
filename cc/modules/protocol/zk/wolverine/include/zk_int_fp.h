...
// #include "emp-wolverine-fp/zk.h"
// #include "emp-wolverine-fp/conversion.h"
// #include "emp-wolverine-fp/zk_fp_exec.h"
// #include "emp-wolverine-fp/zk_fp_exec_prover.h"
// #include "emp-wolverine-fp/zk_fp_exec_verifier.h"
// #include "emp-wolverine-fp/int_fp.h"
...
// #define LOCAL_SIMULATE_V2 false
...
#define ZKFloatToFixPoint(a) \
  (int64_t)(((int64_t)(a) << ZK_FLOAT_SIZE) + \
        (int64_t((a - (int64_t)(a)) * (1L << ZK_FLOAT_SIZE))))
...
public:
	...
	static inline bool batch_reveal(const ZkIntFp* inputs, size_t size) {
		...
		if(zk_party_id == ALICE) {
			((ZKFpExecPrv<BoolIO<NetIO>>*)(ZKFpExec::zk_exec))->io->send_data(plains.data(), size*sizeof(uint64_t));
		} else {
			((ZKFpExecPrv<BoolIO<NetIO>>*)(ZKFpExec::zk_exec))->io->recv_data(plains.data(), size*sizeof(uint64_t));
		}
	...
	bool open(uint64_t & res) {
		...
		if(zk_party_id == ALICE) {
			((ZKFpExecPrv<BoolIO<NetIO>>*)(ZKFpExec::zk_exec))->io->send_data(&plain, sizeof(uint64_t));
		} else {
			((ZKFpExecPrv<BoolIO<NetIO>>*)(ZKFpExec::zk_exec))->io->recv_data(&plain, sizeof(uint64_t));
		}
		res = plain;
	...
	// substract
	ZkIntFp operator-(const ZkIntFp& rhs) const {
		return zk_fp_sub(*this, rhs, zk_party_id);
  }
	...
	// less
	ZkIntFp operator<(const ZkIntFp& rhs) const {
		return zk_fp_less(*this, rhs, zk_party_id);
  }

	// less-equal
	ZkIntFp operator<=(const ZkIntFp& rhs) const {
		return zk_fp_less_equal(*this, rhs, zk_party_id);
  }

	// equal
	ZkIntFp operator==(const ZkIntFp& rhs) const {
		return zk_fp_equal(*this, rhs, zk_party_id);
  }
	...
	//greater
	ZkIntFp operator>(const ZkIntFp& rhs) const {
		return zk_fp_greater(*this, rhs, zk_party_id);
  }

	// negate
	ZkIntFp operator-() const {
    return zk_fp_neg(*this, zk_party_id);
  }
	...
	static inline void zk_fp_sub(const ZkIntFp *lhs, const ZkIntFp *rhs, int size, vector<ZkIntFp> &res, int party)
	{
		if (res.size() != size)
			res.resize(size);
		for (int i = 0; i < size; ++i)
		{
			...
			if (ALICE == party)
			{
				...
				__uint128_t mac = mod((res[i].value & 0xFFFFFFFFFFFFFFFFULL) + (pr - (rhs[i].value & 0xFFFFFFFFFFFFFFFFULL)), pr);
	...
	static inline void zk_fp_greater(const ZkIntFp *lhs, const ZkIntFp *rhs, int size, int party, vector<ZkIntFp> &out)
	{
		...
		arith2bool<BoolIO<NetIO>>(bin.data(), delta.data(), size);

		Integer zero(ZK_INT_LEN, 0, PUBLIC), one(ZK_INT_LEN, 1, PUBLIC);
		vector<Integer> greater_ret(size, Integer(ZK_INT_LEN, 0, PUBLIC));
		Integer pr_half(ZK_INT_LEN, (uint64_t)((pr >> 2) + 1), PUBLIC); // pr/2+1 is the biggest negative value
		for (size_t i = 0; i < size; ++i)
		{
			Integer equal_cmp = zero.select(bin[i].equal(zero), one);
			Integer less_equal = zero.select(bin[i].geq(pr_half), one) | equal_cmp; //less_equal = less | equal
		...
		if (out.size() != size)
			out.resize(size);
		bool2arith<BoolIO<NetIO>>(out.data(), greater_ret.data(), size);
		sync_zk_bool<BoolIO<NetIO>>();
	}
	...
	static inline void zk_fp_greater_equal(const ZkIntFp *lhs, const ZkIntFp *rhs, int size, int party, vector<ZkIntFp> &out)
	{
		vector<ZkIntFp> delta;
		zk_fp_sub(lhs, rhs, size, delta, party);
		...
		for (size_t i = 0; i < size; ++i)
		{
			greater_equal_ret[i] = one - zero.select(bin[i].geq(pr_half), one);// greater_equal = 1 - less
			// cout << "greater_equal[" << i << "]: " << greater_equal_ret[i].reveal<uint64_t>(PUBLIC) << endl;
		}

		if (out.size() != size)
			out.resize(size);
		bool2arith<BoolIO<NetIO>>(out.data(), greater_equal_ret.data(), size);
		sync_zk_bool<BoolIO<NetIO>>();
	}
	...
	static inline void zk_fp_equal(const ZkIntFp *lhs, const ZkIntFp *rhs, int size, int party, vector<ZkIntFp> &out)
	{
		vector<ZkIntFp> delta;
		zk_fp_sub(lhs, rhs, size, delta, party);
		...
		if (out.size() != size)
			out.resize(size);
		bool2arith<BoolIO<NetIO>>(out.data(), zk_equal.data(), size);
		sync_zk_bool<BoolIO<NetIO>>();
	}
	...
	static inline void zk_fp_less(const ZkIntFp *lhs, const ZkIntFp *rhs, int size, int party, vector<ZkIntFp> &out)
	{
		vector<ZkIntFp> delta;
		zk_fp_sub(lhs, rhs, size, delta, party);
		...
		if (out.size() != size)
			out.resize(size);
		bool2arith<BoolIO<NetIO>>(out.data(), less_ret.data(), size);
		sync_zk_bool<BoolIO<NetIO>>();
	}
	...
	static inline void zk_fp_less_equal(const ZkIntFp *lhs, const ZkIntFp *rhs, int size, int party, vector<ZkIntFp> &out)
	{
		vector<ZkIntFp> delta;
		zk_fp_sub(lhs, rhs, size, delta, party);
		...
		{
			Integer equal_cmp = zero.select(bin[i].equal(zero), one);
			less_equal_ret[i] = zero.select(bin[i].geq(pr_half), one) | equal_cmp; //less_equal = less | equal
			// cout << "less_equal[" << i << "]: " << less_equal_ret[i].reveal<uint64_t>(PUBLIC) << endl;
		}

		if (out.size() != size)
			out.resize(size);
		bool2arith<BoolIO<NetIO>>(out.data(), less_equal_ret.data(), size);
		sync_zk_bool<BoolIO<NetIO>>();
	}
