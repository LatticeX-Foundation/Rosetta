...
using namespace emp;
using namespace std;
// class rosetta::zk::ZkIntFp;

// typedef emp::NetIO ZK_NET_IO;
typedef BoolIO<NetIO> ZK_NET_IO;
...
#define FloatToFixPoint(a) \
  (int64_t)(((int64_t)(a) << ZK_F) + \
        (int64_t((a - (int64_t)(a)) * (1L << ZK_F))))
...
static inline void convert_string_to_double(const vector<std::string>& a, double* b, bool human = humanable) {
  size_t size = a.size();
...
static inline void convert_string_to_double(const vector<std::string>& a, vector<double>& b, bool human = humanable) {
  size_t size = a.size();
  b.resize(size);
...
// adapter for internal arith2bool<BoolIO<NetIO>> with too large size. 
inline void zk_arith2bool(const vector<ZkIntFp>& in, vector<Integer>& out, int size) {
  ...
  if (size <= ONCE_MAX) {
    arith2bool<BoolIO<NetIO>>(out.data(), (IntFp*)in.data(), size);
    // for debuging:
    ...
    for(int i = 0; i < iters; ++i) {
      log_debug << i << "-th iter arith2bool<BoolIO<NetIO>> [" << i * ONCE_MAX << ", " << (i+1)* ONCE_MAX << ")";
      vector<ZkIntFp> curr_iter_in(in.cbegin() + i * ONCE_MAX, in.cbegin() + (i+1)* ONCE_MAX);
      curr_iter_out.clear();
      curr_iter_out.resize(ONCE_MAX);
      arith2bool<BoolIO<NetIO>>(curr_iter_out.data(), (IntFp*)curr_iter_in.data(), ONCE_MAX);
    ...
    if(leftover_num > 0) {
      log_debug << "remaining arith2bool<BoolIO<NetIO>> [" << iters * ONCE_MAX << ", " << size << ")";
      vector<ZkIntFp> curr_iter_in(in.cbegin() + iters * ONCE_MAX, in.cend());
      curr_iter_out.clear();
      curr_iter_out.resize(leftover_num);
      arith2bool<BoolIO<NetIO>>(curr_iter_out.data(), (IntFp*)curr_iter_in.data(), leftover_num);
      out.insert(out.end(), curr_iter_out.begin(), curr_iter_out.end());
      sync_zk_bool<BoolIO<NetIO>>();
      // for debuging:
      // uint64_t plain_fp_value = 0;
  ...
  log_debug << "batch arith2bool<BoolIO<NetIO>> ok, costing " << switch_time/1000 << " for party " << zk_party_id << endl;
}
...
/*
  ...
  (return 0 mean success, otherwise is fail)
*/
int _inner_proof_and_check(const std::vector<ZkIntFp>& a, const std::vector<ZkIntFp>& a_prime, uint64_t diff_bound = _FLOAT_SCALE_);
int _inner_proof_and_check(const ZkIntFp* a, const ZkIntFp* a_prime, size_t size, uint64_t diff_bound = _FLOAT_SCALE_);
...
*/
int _inner_div_proof_and_check(const std::vector<ZkIntFp>& a, const std::vector<ZkIntFp>& b, const std::vector<ZkIntFp>& c); 
...
// Attention! Please call sync_zk_bool<BoolIO<NetIO>>() before and after calling this function!
static inline void zk_max(const std::vector<ZkIntFp>& in_a, const std::vector<ZkIntFp>& in_b, std::vector<ZkIntFp>& out_c) {
  ...
  arith2bool<BoolIO<NetIO>>(bin_a.data(), (IntFp*)in_a.data(), vec_size);
  // sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(bin_b.data(), (IntFp*)in_b.data(), vec_size);
  // sync_zk_bool<BoolIO<NetIO>>();
  ...
  // sync_zk_bool<BoolIO<NetIO>>();
  bool2arith<BoolIO<NetIO>>(out_c.data(), bin_c.data(), vec_size);
  // sync_zk_bool<BoolIO<NetIO>>();
}
...
static inline uint64_t egcd(const uint64_t& a, const uint64_t& p, uint64_t& inverse_a, uint64_t& placeholder_p) {
  inverse_a = 1;
  placeholder_p = 0;
