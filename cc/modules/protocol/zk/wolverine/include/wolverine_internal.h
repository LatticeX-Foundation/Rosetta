...
static inline void wolverine_truncate(ZkIntFp* in, size_t size) {
  ...
  arith2bool<BoolIO<NetIO>>(bc.data(), (ZkIntFp*)in, size);
...
static inline void wolverine_relu(const vector<ZkIntFp>& in, vector<ZkIntFp>& out, int scale_count=1) {
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(bin.data(), (IntFp*)in.data(), size);
  ...
  // sync_zk_bool<BoolIO<NetIO>>();
  // std::cout << "arith2bool<BoolIO<NetIO>> ok." << endl;
  ...
  // sync_zk_bool<BoolIO<NetIO>>();
  out.resize(size);
  bool2arith<BoolIO<NetIO>>((IntFp*)out.data(), relu.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
}
...
static inline void wolverine_relu_prime(const ZkIntFp* in, size_t size, vector<ZkIntFp>& output) {
  ...
  output.resize(size);
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(bin.data(), (IntFp*)in, size);
  // std::cout << "arith2bool<BoolIO<NetIO>> ok." << endl;
  Integer zero(ZK_INT_LEN, 0, PUBLIC);
  Integer one(ZK_INT_LEN, 1, PUBLIC);
  ...
  bool2arith<BoolIO<NetIO>>((IntFp*)output.data(), relu_prime.data(), size);
  sync_zk_bool<BoolIO<NetIO>>();
}
...
static inline void wolverine_max_matrix(const vector<ZkIntFp>& in, size_t rows, size_t cols, vector<ZkIntFp>& out) {
  ...
  sync_zk_bool<BoolIO<NetIO>>();
  arith2bool<BoolIO<NetIO>>(input_bin.data(), (IntFp*)in.data(), size);
  // sync_zk_bool<BoolIO<NetIO>>();
  ...
  // sync_zk_bool<BoolIO<NetIO>>();
  bool2arith<BoolIO<NetIO>>((IntFp*)out.data(), bin_c.data(), rows);
  sync_zk_bool<BoolIO<NetIO>>();
...
static inline int wolverine_matmul_fp_with_proof(const ZkMatrix& a, const ZkMatrix& b, ZkMatrix& c) {
  log_debug << "party: " << zk_party_id << " : wolverine_matmul_fp_with_proof, (" << a.rows() << "," << a.cols() << ") * (" << b.rows() << "," << b.cols() << "), start..." << std::endl;
...
static inline int wolverine_matmul_fp_with_proof_r_const(const ZkMatrix& a, const DoubleMatrix& b, ZkMatrix& c) {
  log_debug << "party: " << zk_party_id << " : wolverine_matmul_fp_with_proof, (" 
          << a.rows() << "," << a.cols() << ") * (" 
          << b.rows() << "," << b.cols() << "), B is const start..." << std::endl;
  assert(a.size() != 0 && b.size() != 0);
  if (c.size() == 0)
    c.resize(a.rows(), b.cols());
