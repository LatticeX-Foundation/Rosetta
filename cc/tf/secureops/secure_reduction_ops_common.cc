...
TensorShape ReductionHelper_RTT::out_reshape() const {
  TensorShape shape;
  for (auto size : out_reshape_)
    shape.AddDim(size);
  return shape;
}

// The final output shape must be allocated with this shape.
TensorShape ReductionHelper_RTT::out_shape() const {
  TensorShape shape;
...
TensorShape ReductionHelper_RTT::shuffled_shape() {
...
gtl::InlinedVector<int32, 8> ReductionHelper_RTT::permutation() {
  const int dims = data_reshape_.size();
...
Status SimplifyHelper(const Tensor& data, const Tensor& axis, gtl::InlinedVector<bool, 4>& bitmap) {
  ...
  for (int64 i = 0; i < axis.NumElements(); ++i) {
    ...
    if (index < -data.dims() || index >= data.dims()) {
      return errors::InvalidArgument(
        "Invalid reduction dimension (", index, " for input with ", data.dims(), " dimension(s)");
    }
...
Status ReductionHelper_RTT::Simplify(const Tensor& data, const Tensor& axis, const bool keep_dims) {
  ...
  if (dim_index >= data.dims()) {
    ...
    for (size_t i = reduce_first_axis_ ? 1 : 0; i < data_reshape_.size(); i += 2) {
