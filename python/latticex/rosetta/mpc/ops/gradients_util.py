        ...
        with gradients_util._maybe_colocate_with(y.op, gradient_uid, colocate_gradients_with_ops):
            if grad_y is None:
                ...
                new_grad_ys.append(
                    array_ops.fill(
                        array_ops.shape(y),
                        constant(1.0, dtype=y.dtype, name="grad_ys_%d" % i)))
                continue
...
# overide tensorflow _DefaultGradYs with MpcDefaultGradYs
gradients_util._DefaultGradYs = MpcDefaultGradYs

# overide tensorflow _IsBackpropagatable with MpcIsBackpropagatable
gradients_util._IsBackpropagatable = MpcIsBackpropagatable
