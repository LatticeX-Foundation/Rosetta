...
# ===========================
# define tf mpc sub grad func
# ===========================
def test_sub_grad(X, Y, out_g, protocol="Helix"):
    ...
    # ===========================
    print("===========================")
    print("run mpc sub(X + Y) grad")
    mpc_Z = cb.SecureSub(X, Y)
...
test_sub_grad(X, Y, [1.0, -1.0])
test_sub_grad(X2, Y2, [[1., 1.], -2.0])

# test snn grad op
print("run snn protocol...")
test_sub_grad(X, Y, [1.0, -1.0], protocol="SecureNN")
test_sub_grad(X2, Y2, [[1., 1.], -2.0], protocol="SecureNN")
...
Writer = tf.summary.FileWriter("log/mpcsub", tf.get_default_graph())
Writer.close()
cb.deactivate()