...
file_x = '../dsets/' + node_id + "/reg_train_x_feature.csv"
file_y = '../dsets/' + node_id + "/reg_train_y_feature.csv"
real_X, real_Y = rtt.PrivateDataset(data_owner=(
    0, 2, 'p9'),dataset_type = rtt.DatasetType.FeatureAligned, task_id = task_id).load_data(file_x, file_y, header=None)
# ######################################## difference from tensorflow
...
with tf.Session(task_id = task_id) as sess:
    ...
    #print("Y_pred:", Y_pred)
    #print("reveal:", sess.run(rtt.SecureReveal(Y_pred)))
    print('reveal:', sess.run(reveal_Y, feed_dict={X: real_X, Y: real_Y}))
