...
with tf.compat.v1.Session() as mpc_check_result_sess:
    ...
    if ((abs(np.array(a_out, dtype=np.float) - num_a) < precision_target).all() and \
        (abs(np.array(b_out, dtype=np.float) - num_b) < precision_target).all()):
        print("num_a and num_b restored correctly!")
    else:
        print("num_a or num_b restored failed!")
        print("You may check the `SAVER_MODE` and `RESTORE_MODE` in OCNFIG.json be in {1, 3, 5, 7}")
