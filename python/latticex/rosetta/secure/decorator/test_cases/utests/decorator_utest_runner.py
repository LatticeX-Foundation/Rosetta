...
# TODO: [kelvin]
class UtestRunner(object):
    def __init__(self, *args):
        super(UtestRunner, self).__init__(*args)
...
def _test_binary_sample_call(test_case, index, loss_precision=10):
    if index < len(sample_test_data_array2) and len(sample_test_data_array2[index]) == 1:
        ...
        expect_outputs = test_case.run_local(in0, in1, test_case.op_name)
         ...
         print("expect-outputs: {}, type: {}".format(expect_outputs,
                                                      type(expect_outputs)))
           ...
           if loss_precision != None:
                if _is_round_equal(test_case, outputs.tolist(), expect_outputs.tolist(), loss_precision):
                    print("round equal: {} ~= {}".format(
                        outputs.tolist(), expect_outputs.tolist()))
                else:
...
def _test_unary_reduce_sample_call(test_case, index, is_reduce=False, loss_precision=None):
    if index < len(sample_test_data_array):
        ...
        outputs = test_case.run_players(in0, in1, test_case.op_name)
         expect_outputs = test_case.run_local(
              in0, zeros_in1, test_case.op_name)

          print("**local-outputs: {}, type: {}".format(expect_outputs,
                                                        type(expect_outputs)))
           print("**mpc-outputs: {}, type: {}".format(outputs, type(outputs)))
