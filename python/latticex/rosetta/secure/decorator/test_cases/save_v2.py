...
with tf.compat.v1.Session() as mpc_save_sess:
    ...
    # print([na, nb])
    mpc_save_v2_op = rtt.SecureSaveV2(SAVED_FILE_PREFIX, # the filename_tensor
...
# Note that the following should be runable only if you set the SAVER_MODE as '0' and RESTORE_MODE as '0'!
config_str = rtt.get_protocol_config()
curr_conf = json.loads(config_str)

if curr_conf["MPC"]["SAVER_MODE"] != 0 or curr_conf["MPC"]["RESTORE_MODE"] != 0:
    print("To run the following, you should set SAVER_MODE as '0' and RESTORE_MODE as '0' in youe CONFIG.json")
    exit()
